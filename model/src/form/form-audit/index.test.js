...
describe('schema tests', () => {
  ...
  describe('FORM_UPDATED output changes', () => {
    /**
     * @param {object} outputChanges
     */
    const messageWithOutputChanges = (outputChanges) => ({
      ...REPLACE_DRAFT,
      data: { ...REPLACE_DRAFT.data, outputChanges }
    })

    it('should validate an added output', () => {
      const message = messageWithOutputChanges({
        added: [
          {
            emailAddress: 'tester@defra.gov.uk',
            audience: 'human',
            version: '1'
          }
        ]
      ...
      expect(() =>
        Joi.attempt(message, messageSchema, { abortEarly: false })
      ).not.toThrow()
    })

    it('should validate an amended output', () => {
      const message = messageWithOutputChanges({
        updated: [
          {
            previous: {
            ...
            new: {
              ...
              audience: 'machine',
              version: '2',
              condition: 'e2b7f9c1-1b2c-4c3d-9e4f-5a6b7c8d9e0f'
            }
          }
        ]
      })
    ...
    it('should validate outputs removed alongside their condition', () => {
      const message = {
        ...REPLACE_DRAFT,
        data: {
          ...REPLACE_DRAFT.data,
          s3Meta: undefined,
          requestType: 'DELETE_CONDITION',
          payload: { conditionId: 'e2b7f9c1-1b2c-4c3d-9e4f-5a6b7c8d9e0f' },
          outputChanges: {
            removed: [
              {
                emailAddress: 'tester@defra.gov.uk',
    ...
    it('should reject an output missing its audience', () => {
      const message = messageWithOutputChanges({
        added: [{ emailAddress: 'tester@defra.gov.uk', version: '1' }]
      })

      expect(() =>
        Joi.attempt(message, messageSchema, { abortEarly: false })
      ).toThrow('"data.outputChanges.added[0].audience" is required')
    })

    it('should reject output changes recording nothing', () => {
      const message = messageWithOutputChanges({})

      expect(() =>
        Joi.attempt(message, messageSchema, { abortEarly: false })
      ).toThrow(
        '"data.outputChanges" must contain at least one of [added, updated, removed]'
      )
    })
  })
