...
describe('condition model', () => {
  ...
  describe('special character component names', () => {
    let underTest: ConditionsModel
    ...
    test('component names with dashes are properly handled in expressions', () => {
      underTest.add(
        new Condition(
          new ConditionField('badger-field', ComponentType.TextField, 'Badger'),
          OperatorName.Is,
          new ConditionValue('Zebras')
        )
      )

      expect(underTest.toExpression()).toBe("_field_badger_field == 'Zebras'")
    })

    test('component names with plus sign are properly handled', () => {
      underTest.add(
        new Condition(
          new ConditionField(
            'squiffy+field',
      ...
      expect(underTest.toExpression()).toBe("_field_squiffy_field != 'Donkeys'")
    })

    test('component names with asterisk are properly handled', () => {
      underTest.add(
        new Condition(
          new ConditionField(
            'monkeys*field',
      ...
      expect(underTest.toExpression()).toBe("_field_monkeys_field == 'Zebras'")
    })

    test('component names with period are properly handled', () => {
      underTest.add(
        new Condition(
          new ConditionField(
            'duration.field',
      ...
      expect(underTest.toExpression()).toBe('_field_duration_field >= 10')
    })

    test('complex conditions with special character field names work correctly', () => {
      underTest.add(
        new Condition(
          new ConditionField('badger', ComponentType.TextField, 'Badger'),
          OperatorName.Is,
      ...
      expect(underTest.toExpression()).toBe(
        "(badger == 'Zebras' and _field_squiffy_field == 'Donkeys') or under18"
      )
    })
  })
})
